...
class RefundedPayment implements \Teg\Types\Interface\InitObject
{
    ...
    public function __construct($request)
    {
        ...
        $this->currency = $request->currency ?? 'XTR';
        $this->total_amount = $request->total_amount ?? 0;
        $this->invoice_payload = $request->invoice_payload ?? '';
        $this->telegram_payment_charge_id = $request->telegram_payment_charge_id ?? '';

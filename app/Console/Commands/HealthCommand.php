...
{
    protected $signature = 'teg:health {--bot= : Имя конкретного бота для проверки} {--no-ssl : Отключить проверку SSL сертификатов} {--verbose-errors : Показать подробные ошибки из логов}';
    protected $description = 'Проверка состояния TegBot';

    public function handle()
    {
        $this->info('🔍 Проверка состояния мультиботной системы TegBot');
        $this->newLine();
